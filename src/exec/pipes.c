...
/*   Updated: 2024/02/22 21:15:43 by nledent          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_input(t_list_cmd *bloc_data, t_redir *redir)
{
	int	input;
...
static int	init_output(t_list_cmd *bloc_data, t_redir *redir)
{
	int		output;
...
static int	redirections(t_list_cmd *bloc_data)
{
	...
	{
		if (next->in_out == 0)
			r_redir_in = init_input(bloc_data, next->file_path);
		else if (next->in_out == 1)
			r_redir_out = init_output(bloc_data, next);
...
void	pipes_redir(t_sh_data *sh, int p_out[2], int p_in[2], t_list_cmd *bloc)
{
	...
	if (r_redir >= 4)
	{
		free_cmd(sh->cmd_bloc1);
		free_cmd(sh->env_var1);
		if (bloc->id == 0)
			close_pipes(p_in);
		close_pipes(p_out);
		exit(1);
